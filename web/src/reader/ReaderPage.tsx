...
export function ReaderPage({ theme }: Props) {
    ...
    const open = async () => {
      try {
        ...
        if (book.source_format === "mobi") {
          ...
          if (status.status === "pending" || status.status === "failed") await api.startConversion(id);
        ...
        if (!cancelled) {
          setDetail(book);
          setProgress(saved);
          setEncoding(saved?.position.type === "txt" ? saved.position.encoding : "auto");
          setTextNavigation([]);
  ...
  return <main className="reader-shell">
    ...
    {settingsOpen && <ReaderSettingsPanel detail={detail} settings={settings} setSettings={setSettings} comicSettings={comicSettings} setComicSettings={setComicSettings} encoding={encoding} onEncodingChange={setEncoding} text={text} />}
    ...
    {text
      ? <FoliateTextReader detail={detail} progress={progress} settings={settings} theme={theme} encoding={encoding} navigationRequest={navigationRequest} onProgress={save} onNavigationChange={(items, activeId) => { setTextNavigation(items); if (activeId) setActiveNavigationId(activeId); }} />
      : <ComicReader detail={detail} progress={progress} settings={comicSettings} navigationRequest={navigationRequest} onCurrentPageChange={(page) => setActiveNavigationId(String(page))} onProgress={save} />}
  </main>;
}
